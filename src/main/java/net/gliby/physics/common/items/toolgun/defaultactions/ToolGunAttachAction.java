...
import net.gliby.physics.common.items.toolgun.ToolGunHit;
...
public class ToolGunAttachAction implements IToolGunAction {
	...
	public boolean use(PhysicsWorld physicsWorld, EntityPlayerMP player, Vector3f blockLookAt) {
		...
		if (ray.getCollisionObject() != null && ray.hasHit()) {
			...
			if (body != null) {
				Vector3f localHit = new Vector3f(body.getPosition().getX(), body.getPosition().getY(),
						body.getPosition().getZ());
				localHit.sub(ray.getHitPointWorld());
				ToolGunHit hit;
				if ((hit = hits.get(player.getEntityId())) != null) {
					Transform transformA = new Transform();
					transformA.setIdentity();
					transformA.origin.set(localHit);
					...
					hits.put(player.getEntityId(), new ToolGunHit(localHit, body));
			}
		}
		return true;
	}
