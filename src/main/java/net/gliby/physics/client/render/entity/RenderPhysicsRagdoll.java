...
public class RenderPhysicsRagdoll extends RenderPhysics {
	 ...
	 * @param renderManager
	 */
	public RenderPhysicsRagdoll(RenderManager renderManager) {
		super(renderManager);
	}

	public void draw(Entity uncast, double worldX, double worldY, double worldZ, float partialTick) {
		/*EntityPhysicsRagdoll entity = (EntityPhysicsRagdoll) uncast;
		Vector3f world = RenderUtilities.getWorldTranslation(Minecraft.getMinecraft(), partialTick);
		glPushMatrix();
		glTranslatef(-world.x + 0.5f, -world.y + 0.5f, -world.z + 0.5f);
		glDisable(GL_TEXTURE_2D);
		for (int i = 0; i < entity.networkBounds.length; i++) {
			Vector3f position = new Vector3f(entity.renderPositions[i]);
			Quat4f rotation = entity.renderRotations[i];
			Vector3f halfExtent = new Vector3f(entity.networkBounds[i]);
	...
	/* (non-Javadoc)
	 * @see net.gliby.physics.client.render.entity.RenderPhysics#getRenderHitPoint(net.gliby.physics.common.entity.EntityPhysicsBase, float)
	 */
	@Override
