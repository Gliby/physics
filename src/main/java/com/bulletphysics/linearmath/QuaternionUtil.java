...
public class QuaternionUtil {
    ...
    public static float getAngle(Quat4f q) {
        float s = 2f * (float) FastMath.acos(q.w);
        return s;
    }
    ...
    public static void setRotation(Quat4f q, Vector3f axis, float angle) {
        float d = axis.length();
        assert (d != 0f);
        float s = (float) FastMath.sin(angle * 0.5f) / d;
    ...
    public static Quat4f shortestArcQuat(Vector3f v0, Vector3f v1, Quat4f out) {
        ...
        float s = (float) FastMath.sqrt((1.0f + d) * 2.0f);
        float rs = 1.0f / s;
    ...
    public static void setEuler(Quat4f q, float yaw, float pitch, float roll) {
        ...
        float sinYaw = (float) FastMath.sin(halfYaw);
        float cosPitch = (float) Math.cos(halfPitch);
        float sinPitch = (float) FastMath.sin(halfPitch);
        float cosRoll = (float) Math.cos(halfRoll);
        float sinRoll = (float) FastMath.sin(halfRoll);
