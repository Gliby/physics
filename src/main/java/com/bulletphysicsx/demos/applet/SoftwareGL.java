...
public class SoftwareGL implements IGL {
    ...
    private void setMatrix(Matrix4f mat) {
        switch (matrixMode) {
            case GL_MODELVIEW:
                gl.setViewMatrix(mat);
                break;
            case GL_PROJECTION:
                gl.setProjMatrix(mat);
                break;
            default:
    ...
    private void mulMatrix(Matrix4f mat) {
        switch (matrixMode) {
            case GL_MODELVIEW:
                gl.mulViewMatrix(mat);
                break;
            case GL_PROJECTION:
                gl.mulProjMatrix(mat);
                break;
            default:
                throw new IllegalStateException();
        }
    ...
    public void glBegin(int mode) {
        switch (mode) {
            case GL_LINES:
                gl.begin(Graphics3D.LINES);
                break;
            case GL_TRIANGLES:
                gl.begin(Graphics3D.TRIANGLES);
                break;
            case GL_QUADS:
                gl.begin(Graphics3D.QUADS);
                break;
            default:
